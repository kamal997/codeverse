  ...
  return (
    <div className="py-10">
      ...
      >
        ...
        />
        {/* Overlay fade (light-only) */}
        <div className="absolute bottom-0 h-40 w-full [mask-image:linear-gradient(to_bottom,transparent,white)] bg-white" />
      </motion.div>
      ...
      <div className="flex lg:flex-row justify-between items-center flex-col mt-20">
        ...
        <div className="flex space-x-2 md:mb-1 mt-2 md:mt-0">
          {product.stack?.map((stack) => (
            <span
              key={stack}
              className="text-xs md:text-xs lg:text-xs bg-gray-50 px-2 py-1 rounded-sm text-[var(--secondary-600)]"
            >
              {stack}
            </span>
          ))}
      ...
      <div className="prose prose-sm md:prose-base max-w-none text-neutral-600 pb-5">
        {product?.content}
      </div>
